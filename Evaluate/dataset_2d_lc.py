...
class DatasetFolder(VisionDataset):
    ...
    def __init__(
        ...
        # Use 1 or 2 to set up cls-to-idx
        # 1. setup cls-to-idx binding by iterating all sub-folders
        # classes, class_to_idx = self.find_classes(self.root, class_num=class_num)
        # 2. setup cls-to-idx binding by predefined cls_to_idx.csv
        self.class_to_idx = {}
        folderIdx = root.find(root.split('/')[3])
        cls_file = root[:folderIdx]
        cls_file = os.path.join(cls_file, 'cls_to_idx.csv')
        if os.path.exists(cls_file):
            cls_df = pd.read_csv(cls_file, sep=',', header=None)
            for _, row in cls_df.iterrows():
                cls_id, cls_name = row
                cls_id = int(cls_id)
                self.class_to_idx[cls_name] = cls_id
        else:
            raise ValueError('cls_to_idx file path is not recovered, please refer to path: ', root)

        # samples = List([path_to_image, index])
        samples = self.make_dataset(
            self.root, self.class_to_idx, extensions, is_valid_file, class_num=class_num
        )
        self.loader = loader
